...
- `@media (max-width: 860px)`: tightens global spacing in `global.css` by reducing `--container-pad` and `.main` padding to `var(--space-6)` while keeping all sections legible.
- `Header.astro @media (max-width: 1100px)`: hides the desktop header nav/CTA stack and switches to the mobile CTA row plus `details.menu` disclosure panel.
...
- Desktop nav stays active above `1100px`; at and below `1100px`, the header switches to a mobile CTA row plus `details.menu` / `.menu__panel`.
...
- Mobile menu uses `var(--shadow-2)` and `--radius-3` to match overall surface language; the disclosure script closes on link click, outside click, `Escape`, and when resizing back above the mobile breakpoint.
- `Watch Live` and `Give` stay visible in the mobile header CTA row and are not duplicated inside the disclosure panel; the panel holds primary nav plus utility links.
